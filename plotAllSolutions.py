...
for pfname in pfnames: 
	...
	plot(time[time>4],param[42]*ones(len(sol))[time>4],color='blue', ls='dashed')
	ax2 = ax1.twinx()
	ax2.plot(time,ipump,color='red')
	ylabel('Ipump (nA)')
	xlim(4,5)
	...
	xlabel('time (sec)')
	xlim(4,5)
	savefig('optsol-'+pfname+'.zoom.png')
